...
st.set_page_config(page_title="Smart hospital by Edgard", page_icon="🏥", layout="wide")
...
DEPT_INFO = {
    ...
    'Gastroenterology': {
        ...
        'desc':'specialises in digestive system and abdonial condition',
    ...
    'Neurology': {
        ...
        'desc':'specialises in brain spine and nervous system condition ',
...
st.markdown("""
<!-- COLOR1 -->
<div style="background:linear-gradient(135deg,#893bff 0%,#17dcff 60%,#c10cdd 100%);
    ...
    <div style="font-size:18px;color:rgba(255,255,255,0.85);font-weight:400;">
        find the right apartemen for your syntem
    </div>
</div>
""", unsafe_allow_html=True)
...
with st.form("triage_form"):
    ...
    st.markdown("""
    <!-- COLOR2 -->
    <div style="background:#f609ee;border:1px solid #cbdc0d;border-radius:14px;
        ...
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
            ...
            <span style="font-size:16px;font-weight:600;color:#105efa;">what are your main syntems</span>
    ...
    st.markdown("""
    <!-- COLOR3 -->
    <div style="background:#008df1;border:1px solid #0d2fdc;border-radius:14px;
                padding:20px 24px;margin-bottom:20px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
            <span style="background:#630af2;color:rgb(255, 255, 255);border-radius:8px;
                         padding:4px 10px;font-size:12px;font-weight:600;">2</span>
            <span style="font-size:16px;font-weight:600;color:#105efa;">how long have you have this synthem</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_cc:
        chief_complaint = st.selectbox("chief complain", options=list(cc_map.keys()))
    with col_dur:
    ...
    st.markdown("""
    <!-- COLOR4 -->
    <div style="background:#0022fe;border:1px solid #075aff;border-radius:14px;
                padding:20px 24px;margin-bottom:20px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
            <span style="background:#6206f7;color:rgb(85, 4, 236);border-radius:8px;
    ...
    with col_temp:
        temperature_level = st.selectbox("temperaturelevel", options=list(temp_map.keys()), index=1)
    with col_hr:
        heart_rate_level  = st.selectbox("heartratelevel", options=list(hr_map.keys()), index=1)
    ...
    st.markdown("""
    <!-- COLOR5 -->
    <div style="background:#4409f6;border:1px solid #710ddc;border-radius:14px;
                padding:20px 24px;margin-bottom:20px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
    ...
    st.markdown("""
    <!-- COLOR6 -->
    <div style="background:#f609e2;border:1px solid #cbdc0d;border-radius:14px;
                padding:20px 24px;margin-bottom:20px;">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px;">
            <span style="background:4#25f20a;color:white;border-radius:8px;
                         padding:4px 10px;font-size:12px;font-weight:600;">5</span>
            <span style="font-size:16px;font-weight:600;color:#105efa;">patient info</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
...
if submitted:
    ...
    <div style="font-size:14px;color:#6b7280;margin-bottom:1.5rem;">based in the information you provided</div>
    """, unsafe_allow_html=True)
    ...
    with res_col:
        ...
        <div style="background:{info['bg']};border:1.5px solid {info['border']};
            ...
            <div style="margin-top:20px;padding:12px 16px;background:rgba(0,0,0,0.05);
                ...
                ⚠️ this ai suggestion, not a medical diagnosis please consult a docter forfurther evaluation
            </div>
        </div>
        """, unsafe_allow_html=True)
